...
Simple maintainer flow:

1. While the issue is open, use `status:*` labels to track triage and work state.
2. Open a PR to `main` with `Closes #...` in the PR body and the correct changelog label on the PR.
3. When the PR merges, GitHub closes the issue and automation removes `status:*` labels and adds `release: pending`.
4. When the release is published, automation removes `release: pending` and adds `release: shipped`.

In short:

- `status:*` = open issue workflow state
- `release:*` = closed issue delivery state
- PR labels = generated release note categories
...
`ChoreEngine.has_missed_occurrence_between` is the single authority for "was an occurrence missed between X and Y"; `ChoreEngine.build_schedule_config` is the single source of how a chore maps onto a recurrence. Callers supply the anchor, because the correct one differs by caller (badge streaks anchor on the badge's advance day, chore streaks and achievement streaks on the previous completion).

**An unscheduled chore is evaluated as daily.** A chore with `FREQUENCY_NONE` and no due date has no rhythm to keep, so the helper treats it as daily: consecutive days continue the streak, a skipped day breaks it. This is an **inferred intent**, not a derived fact — the user never specified one — and it is the deliberate choice of decision 18. It fixes a pathological case: when such a chore could never report a miss, a badge scoped only to it would advance once and then freeze forever, because later days had nothing approved and nothing detectably missed. The rule lives in the helper, never inline in a caller.

**An unreadable schedule counts as a miss, for every caller.** `has_missed_occurrence_between` returns `True` when the schedule cannot be evaluated. The two callers used to diverge (a chore streak broke, a badge streak held) via an `unusable_schedule_counts_as_miss` flag; that flag is **removed** so the policy cannot drift apart again. Unreadable data surfaces as a lost streak rather than hiding behind a preserved one — a silent hold would mask the defect. If a future caller needs the opposite, that is a deliberate change to this policy and not a new parameter.

**Unreadable *data* is treated the same way as an unreadable schedule.** A positive streak implies a completion timestamp exists, so an absent or unparseable value is a defect and breaks the streak rather than preserving it.
